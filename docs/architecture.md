...
`src/algorithms/bsp.py` содержит data-only BSP primitives для будущей процедурной структуры замка: `RectInt`, `BSPNode` и `BSPGenerator`.
...
`CastleAssaultScene` пока остаётся на статической карте. Подключение procedural castle layout будет отдельным шагом.
...
- диалоги;
- интеграция BSP-генерации в `CastleAssaultScene`;
