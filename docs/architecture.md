...
Внешние production assets не обязательны: если файл изображения отсутствует, `ResourceManager` возвращает сгенерированный placeholder вместо падения.

`ResourceManager` не содержит sound loading, font loading, sprite sheets, animation controller или asset manifest.
...
`WALL`, `WATER` и `FOREST` считаются blocking.

`FLOOR`, `GRASS`, `DIRT`, `ROAD`, `RUINS_FLOOR` и `BRIDGE` считаются walkable.
...
`TileMap.draw()` может рисовать цветные тайлы напрямую или использовать переданный `ResourceManager` для tile surfaces.
