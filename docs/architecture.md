...
Создаёт окно, `InputManager`, `GameState`, `EventBus`, `InfluenceSystem`, `RegionLiberationSystem`, `ResourceManager`, `SceneManager`, регистрирует `MainMenuScene`, `WorldMapScene`, `RegionScene`, `CastleAssaultScene` и `PauseScene`, затем запускает цикл `handle_events -> update -> draw`.
...
Autosave вызывается после world-state events: `EnemyKilledEvent`, `OutpostClearedEvent`, `SupplyCacheDestroyedEvent`, `QuestCompletedEvent` и `RegionLiberatedEvent`.
...
Минимальный `ResourceManager` кэширует загруженные изображения, runtime animation frames и generated placeholder surfaces.

Сейчас есть первый набор game-ready PNG в `assets/images/`: ordinary tiles, castle tiles, static entity icons, sliced player/enemy walk frames и sliced player attack frames.

`ResourceManager` содержит default mapping от tile ids к PNG и от `Sprite.asset_key` к PNG.

Если файл изображения отсутствует, `ResourceManager` возвращает сгенерированный placeholder вместо падения.

Для animation frames `ResourceManager` возвращает `None`, если конкретный кадр отсутствует, чтобы `RenderSystem` мог откатиться к static `Sprite`.

`ResourceManager` не содержит sound loading, font loading, `AnimationManager` или asset manifest.

Tile surfaces и entity surfaces создаются как `pygame.Surface` и кэшируются по ключу/размеру/цвету.

Sliced walk frames из `assets/images/entities/player/` и `assets/images/entities/enemy/` используются минимальной idle/walk runtime-анимацией.

Sliced player attack frames используются как visual-only attack animation.

Enemy attack frames могут отсутствовать. В этом случае enemy attack animation request всё равно безопасен, а `RenderSystem` откатывается к static enemy `Sprite`.

### `tools/asset_pipeline/`

Содержит минимальные Pillow-based инструменты подготовки PNG:

- `grid_slicing.py`;
- `slice_tilesheet.py`;
- `slice_spritesheet.py`;
- `process_current_assets.py`;
- `export_castle_preview.py`.

Pipeline работает с raw source art из `assets/source/` и пишет game-ready PNG в `assets/images/`.

`assets/source/`, `assets/tmp/` и локальные raw formats игнорируются через `.gitignore`.

Инструменты asset pipeline не импортируют PyGame напрямую и не являются runtime gameplay system.
...
Создаёт крупную ручную карту Old Ruins, ECS-слой, игрока, несколько групп врагов, два аванпоста, один supply cache, два NPC, системы, HUD, camera follow и debug overlay.

Сама структура обычного региона вынесена в data-only `RegionLayout`: matrix, player spawn tile, enemy spawn tiles с patrol routes, outpost tiles, supply cache tiles и NPC tiles.

`RegionScene` превращает tile coordinates из `RegionLayout` в ECS-сущности через `EntityFactory`.

Карта обычного региона больше viewport, использует camera и содержит разные readable tile types: grass, dirt, road, ruins floor, water, forest и bridge.

Критические проходы ручной карты расширены минимум до двух adjacent walkable tiles.

Может получать `GameState` и показывает название текущего региона в HUD.

HUD в `RegionScene` показывает влияние игрока, влияние врага, счётчики outposts/supply caches/quests/enemies, статус доступности штурма и флаг освобождения региона, если регион уже освобождён.
...
При создании проверяет достижимость player spawn, всех стартовых enemies, всех outposts, supply caches, всех NPC и всех patrol route tiles через `src/algorithms/flood_fill.py`.

Validation также проверяет, что карта больше viewport, использует минимум пять типов тайлов, содержит минимум два outposts, один supply cache, два NPC и семь врагов, а outposts/supply caches/NPC не стоят прямо у spawn. Для supply cache дополнительно проверяются уникальные keys, отсутствие overlap с важными сущностями и наличие nearby guard.

Если игрок побеждён, `R` восстанавливает игрока на spawn tile без полного сброса региона: cleared outposts, completed NPC quests и удалённые enemies сохраняются внутри текущей `RegionScene`.

`CapturePoint` в обычную `RegionScene` не добавлялся.

NPC завершает простое задание после зачистки связанного аванпоста через короткое удержание `E`.

Аванпост очищается через короткое удержание `E`, если игрок рядом и рядом нет живых врагов.

Supply cache уничтожается через короткое удержание `E`, если игрок рядом и рядом нет живых врагов-охранников.

`OutpostSystem`, `SupplyCacheSystem` и `NPCInteractionSystem` используют progress-based hold interaction через `dt`: прогресс растёт только при удержании `E` и сбрасывается, если игрок ушёл, условие не выполнено или рядом появились блокирующие враги.

`OutpostClearedEvent`, `SupplyCacheDestroyedEvent` и `QuestCompletedEvent` по-прежнему публикуются один раз и остаются источником изменения влияния региона через `EventBus` и `InfluenceSystem`.

`RegionScene.export_runtime_state()` отдаёт минимальный runtime snapshot обычного региона.

Snapshot хранит defeated enemy indexes, `cleared_outpost_keys`, `destroyed_supply_cache_keys`, `completed_npc_keys`, player position и player health.

`cleared_outpost_keys`, `destroyed_supply_cache_keys` и `completed_npc_keys` используют layout keys, а не raw ECS objects.

`RegionScene.apply_runtime_state()` применяет snapshot при первом создании cached сцены после Continue.

Legacy snapshot compatibility сохранена: старые `outpost_cleared: true` и `npc_quest_completed: true` применяются к первому outpost/NPC.

Snapshot не восстанавливает path cache, AI memory, attack hitbox timers, partial outpost/supply cache/NPC progress или весь ECS.
...
Default procedural castle layout сейчас 72x48 тайлов, то есть больше viewport.

`CastleAssaultScene` использует `Camera`, поэтому большой замок можно просматривать через движение игрока.

Для диагностики layout хранит короткий `castle_layout_fingerprint`, полученный из матрицы и важных tile coordinates.

Замок использует отдельные tile ids: `CASTLE_FLOOR`, `CASTLE_WALL`, `CRACKED_STONE_FLOOR` и `DARK_CORRIDOR_FLOOR`.
...
Последняя capture point совпадает с `layout.final_room_tile`, поэтому финальная комната участвует в завершении штурма.

До появления Behavior Tree стартовые враги размещаются рядом с capture points как guard-позиции.
...
Содержит dataclass-компоненты: `Position`, `Velocity`, `Collider`, `Renderable`, `Sprite`, `Animation`, `AnimationRequest`, `Health`, `PlayerControlled`, `PlayerDefeated`, `Enemy`, `EnemyAttackState`, `Outpost`, `SupplyCache`, `NPC`, `CapturePoint`, `Dead`, `ChaseBehavior`, `PatrolRoute`, `AttackIntent`, `FacingDirection`, `AttackHitbox`, `MeleeAttack`, `HitFlash`, `DamagePopup`, `TemporaryVisualEffect`.

`Sprite` хранит только `asset_key`.

`Sprite` не хранит `pygame.Surface`, путь к файлу или animation state.

`Renderable` остаётся обязательным fallback-компонентом для размера и цвета.

`Animation` хранит только runtime visual state: `animation_key`, `state`, `direction`, `frame_index`, `frame_timer`, `frame_duration`, `lock_timer` и `lock_duration`.

`AnimationRequest` — одноразовый runtime-компонент для visual-only animation request, например `attack`.

`Animation` и `AnimationRequest` не хранят `pygame.Surface`, не создают animation events, не управляют damage/hitbox timing и не входят в save schema.

`SupplyCache` хранит runtime-only состояние склада снабжения: stable `key`, `destroyed`, `destroy_progress`, `destroy_duration` и `interaction_radius`. В save попадает только key уничтоженного склада внутри runtime snapshot.

`HitFlash`, `DamagePopup` и `TemporaryVisualEffect` используются только для runtime combat feedback и не входят в save schema.

### `src/entities/entity_factory.py`

Создаёт типовые ECS-сущности и добавляет им компоненты.

Сейчас фабрика создаёт игрока с `AttackIntent`, `FacingDirection`, `AttackHitbox` и `MeleeAttack`, базового врага с `ChaseBehavior`, `MeleeAttack`, `AttackHitbox` и `EnemyAttackState`, простой аванпост, supply cache, NPC с простым заданием и точку захвата.

Игрок, враги, аванпосты, supply caches, NPC и точки захвата получают `Sprite` с asset key для sprite-ready rendering.

Игрок и враги дополнительно получают `Animation` для idle/walk runtime-анимации.

Outposts, supply caches, NPC и capture points пока не получают `Animation`.

`Renderable` при этом не удалён и остаётся fallback для прямоугольной отрисовки без `ResourceManager`.
...
Также содержит `InfluenceSystem`, который слушает `EnemyKilledEvent`, `OutpostClearedEvent`, `SupplyCacheDestroyedEvent`, `QuestCompletedEvent` и меняет глобальное влияние регионов через `GameState`.

Supply cache даёт умеренный influence сдвиг `+4/-4`: меньше outpost/NPC, не открывает assault само по себе и сохраняет необходимость combat contribution после полного non-combat objective loop.

Текущий баланс `InfluenceSystem` требует более полного регионального цикла перед штурмом: один outpost, один supply cache, один outpost + quest и все regional objectives без combat contribution не открывают assault.

`CaptureSystem` работает только с ECS и `EventBus`: захватывает точки и публикует события.

Правила `CaptureSystem` не менялись: живой enemy рядом с capture point блокирует progress.
...
`MeleeAttackSystem`, `EnemyAttackSystem`, `OutpostSystem`, `SupplyCacheSystem` и `CaptureSystem` принимают optional spatial index.
...
`AnimationSystem` обновляет idle/walk animation state и visual-only locked attack state через deltaTime.

Если у сущности есть ненулевая `Velocity`, анимация переходит в `walk` и выбирает направление по скорости.

Если скорость нулевая, анимация переходит в `idle`, сбрасывает кадр на `0` и использует предыдущее направление или `FacingDirection`.

`AnimationSystem` сначала потребляет `AnimationRequest`, включает requested state, задаёт visual lock timer и удаляет request.

Пока `lock_timer > 0`, движение не перезаписывает `Animation.state` и `Animation.direction`.

После завершения lock анимация возвращается к обычному idle/walk selection.

`AnimationSystem` не рисует, не загружает изображения, не знает про `ResourceManager`, не меняет `FacingDirection`, не создаёт animation events и не синхронизирует hitbox timing.
...
При реальном попадании игрока по врагу `MeleeAttackSystem` добавляет `HitFlash` и, если подключён `VisualEffectSystem`, создаёт `DamagePopup`.

При принятой атаке игрока `MeleeAttackSystem` создаёт короткий slash effect через optional `VisualEffectSystem`.

При принятой атаке игрока `MeleeAttackSystem` также добавляет `AnimationRequest(state="attack")`.

Эти эффекты не меняют damage, HP, cooldowns, knockback, AI, influence или условия смерти.
...
При старте enemy windup `EnemyAttackSystem` добавляет visual-only `AnimationRequest(state="attack")` на врага.
...
Если `RenderSystem` создан с `ResourceManager` и сущность имеет `Animation`, он сначала пробует получить frame surface по `Animation.animation_key`, `Animation.state`, `Animation.direction` и `Animation.frame_index`.

Если animation frame отсутствует, `RenderSystem` откатывается к static `Sprite`.
...
`RenderSystem` рисует active `HitFlash` как короткий overlay поверх sprite или rectangle fallback.

`VisualEffectSystem` обновляет timers через deltaTime, удаляет завершённые `HitFlash`, `DamagePopup` и `TemporaryVisualEffect`, а также рисует damage popups и slash effects отдельным world-слоем после attack hitboxes и до enemy HP bars.

Combat feel effects являются runtime-only: они не сериализуются в `GameState`, не пишутся в save и пересоздаются только во время текущего боя.

Screen shake пока не подключён, чтобы не трогать camera/physics/HUD offset в этом шаге.

Visual-only attack animation не управляет damage timing, hitbox timing, cooldowns или AI.

Animation events, frame events, combo system, hitbox timing sync и `AnimationManager` всё ещё не реализованы.
...
Содержит dataclass-события игры. Сейчас есть `EnemyKilledEvent`, `OutpostClearedEvent`, `SupplyCacheDestroyedEvent`, `QuestCompletedEvent`, `CapturePointTakenEvent` и `RegionLiberatedEvent`.
...
Содержит тайловую карту, типы тайлов, data-only layout обычного региона и модели глобального состояния регионов.

`src/world/tile_types.py` хранит tile ids:
...
`WALL`, `WATER`, `FOREST` и `CASTLE_WALL` считаются blocking.

`FLOOR`, `GRASS`, `DIRT`, `ROAD`, `RUINS_FLOOR`, `BRIDGE`, `CASTLE_FLOOR`, `CRACKED_STONE_FLOOR` и `DARK_CORRIDOR_FLOOR` считаются walkable.
...
`TileMap.draw()` может рисовать цветные тайлы напрямую или использовать переданный `ResourceManager` для tile PNG surfaces.

Неизвестный tile id рисуется debug/fallback цветом, а не ломает отрисовку.

`src/world/region_layout.py` содержит data-only `RegionLayout` и `create_old_ruins_region_layout()`.

`RegionLayout` не импортирует PyGame, ECS или сцены и хранит только tile coordinates.
...
`CastleLayout.fingerprint()` возвращает короткий hash для диагностики сгенерированной матрицы и важных координат.

`CastleGenerator` создаёт layout через BSP rooms, widened L-shaped corridors и проверку достижимости важных точек через существующий Flood fill / BFS.

`CastleGenerator` отвечает за playable layout constraints: final room как final capture point, guard enemy spawn tiles рядом с capture points и более полезные wave spawn tiles.

Замковые комнаты и коридоры используют отдельные visual tile ids: обычный каменный пол, треснувший каменный пол, тёмный коридор и замковую стену.
...
- QuadTree backend;
- Behavior Tree;
- полноценный production art pass;
