...
Минимальный `ResourceManager` кэширует загруженные изображения и generated placeholder surfaces.

Сейчас есть первый набор game-ready PNG в `assets/images/`: ordinary tiles, castle tiles и static entity icons.
...
`ResourceManager` не содержит sound loading, font loading, animation controller или asset manifest.

Tile surfaces и entity surfaces создаются как `pygame.Surface` и кэшируются по ключу/размеру/цвету.

Sliced walk/attack frames уже могут лежать в `assets/images/entities/player/` и `assets/images/entities/enemy/`, но runtime sprite animation пока не подключена.
...
Создаёт крупную ручную карту Old Ruins, ECS-слой, игрока, несколько групп врагов, два аванпоста, два NPC, системы, HUD, camera follow и debug overlay.

Сама структура обычного региона вынесена в data-only `RegionLayout`: matrix, player spawn tile, enemy spawn tiles с patrol routes, outpost tiles и NPC tiles.
...
HUD в `RegionScene` показывает влияние игрока, влияние врага, счётчики outposts/quests/enemies, статус доступности штурма и флаг освобождения региона, если регион уже освобождён.
...
При создании проверяет достижимость player spawn, всех стартовых enemies, всех outposts, всех NPC и всех patrol route tiles через `src/algorithms/flood_fill.py`.

Validation также проверяет, что карта больше viewport, использует минимум пять типов тайлов, содержит минимум два outposts, два NPC и семь врагов, а outposts/NPC не стоят прямо у spawn.
...
Snapshot хранит defeated enemy indexes, `cleared_outpost_keys`, `completed_npc_keys`, player position и player health.

`cleared_outpost_keys` и `completed_npc_keys` используют layout keys, а не raw ECS objects.
...
Содержит dataclass-компоненты: `Position`, `Velocity`, `Collider`, `Renderable`, `Sprite`, `Health`, `PlayerControlled`, `PlayerDefeated`, `Enemy`, `EnemyAttackState`, `Outpost`, `NPC`, `CapturePoint`, `Dead`, `ChaseBehavior`, `PatrolRoute`, `AttackIntent`, `FacingDirection`, `AttackHitbox`, `MeleeAttack`, `HitFlash`, `DamagePopup`, `TemporaryVisualEffect`.
...
Игрок, враги, аванпосты, NPC и точки захвата получают `Sprite` с asset key для sprite-ready rendering.
...
Текущий баланс `InfluenceSystem` требует более полного регионального цикла перед штурмом: один outpost, один outpost + quest и все regional objectives без combat contribution не открывают assault.
...
Runtime sprite animation и `AnimationManager` всё ещё не реализованы.
