...
- normalization helper для entity animation frames через `tools/asset_pipeline/sprite_normalization.py`;
- validator финальных surface tiles через `tools/asset_pipeline/validate_surface_tiles.py`;
- validator visual footprint entity animation frames через `tools/asset_pipeline/validate_entity_animation_frames.py`;
...
Runtime renderer не содержит special-case scaling для animation frames.
...
Проверить visual footprint entity animation frames:
