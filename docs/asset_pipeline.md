...
- visual stability audit для entity animation frames через `tools/asset_pipeline/audit_entity_animation_visuals.py`;
...
- weak olive/green chroma remnants тоже становятся fully transparent;
...
Bright chroma cleanup сам по себе недостаточен: приглушённые зелёные остатки вроде `(10, 120, 0, 255)`, `(23, 100, 14, 255)` и weak olive pixels вроде `(0, 34, 0, 255)` тоже считаются artifacts для текущих player/enemy animation frames.

После cleanup кадры проходят entity-level palette stabilization:

- для player используется одна stable palette для walk и attack frames;
- для enemy используется одна stable palette для walk frames;
- RGB pixels snap-ятся к ближайшему representative color;
- alpha mask сохраняется;
- dithering не используется;
- лимит validator для final animation frame — до `64` visible RGB colors.
...
Проверить visual stability audit и создать ignored preview sheets:

```bash
.venv/bin/python tools/asset_pipeline/audit_entity_animation_visuals.py
```
...
Animation visual audit preview sheets создаются в `assets/tmp/previews/animation_audit/`.
...
Final animation PNG не должны содержать visible chroma-green pixels, green-dominant opaque remnants, weak green/olive remnants, transparent pixels с ненулевым RGB или low-alpha colored noise.

Final animation PNG также проверяются как runtime-visible `28x28` nearest preview, потому что gameplay renderer рисует entity frames в размер `Renderable`.
