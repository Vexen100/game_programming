...
Surface tileset v2 не добавляет character spritesheets, animations, `AnimationManager` или новые runtime visual systems.
Этот pipeline только готовит PNG и безопасно подключает их к текущему rendering path.
