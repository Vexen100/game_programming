...
Более сложная оптимизация pathfinding, Behavior Tree и SpatialIndex ещё не реализованы.
...
- contextual prompts для outpost и NPC;
- запуск штурма по `C`, если `assault_unlocked == True`.

Это всё ещё ручной прототип без procedural generation, sprite animations и sound.
