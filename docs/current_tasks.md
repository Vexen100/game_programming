...
- игрок, враги, аванпосты и NPC используют placeholder visuals;
- `ResourceManager` может рисовать generated surfaces;
...
- sprite-ready placeholder rendering через `ResourceManager`, `Sprite` и `Renderable` fallback;
...
Assault unlock стал сложнее: outpost alone, one outpost + one NPC quest и полный objective loop без combat contribution больше не открывают штурм.

Ожидаемый путь к unlock теперь требует более полного регионального цикла: два outposts, два NPC quests и combat contribution.

Повторный выбор того же региона на карте больше не сбрасывает текущую `RegionScene` в рамках одного запуска игры: cleared outposts, completed NPC quests, removed enemies, позиция игрока и runtime timers остаются внутри cached scene object.
...
Disk persistence сохраняет `GameState` и минимальный snapshot обычной `RegionScene`: defeated enemy indexes, cleared outpost keys, completed NPC keys, player position и player health.
...
Это всё ещё ручной прототип без production sprites, sprite animations, sound, hit effects, полноценного QuestSystem, диалоговых окон, multi-slot save UI, SettingsScene и procedural ordinary regions.
...
Это не означает, что игра готова. Multi-slot save UI, manual save menu, CastleAssault runtime save, partial region resurgence while away, SettingsScene, Behavior Tree, room theming, doors/traps/decorations, production sprites, animations и sound ещё не реализованы.
