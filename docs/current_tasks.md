...
Это всё ещё ручной прототип: первый static PNG pass, idle/walk animation, visual-only attack animation, normalized animation frame footprint и базовый combat feedback подключены, но полноценный production art pass, animation events, hitbox timing sync, sound, screen shake, полноценный QuestSystem, диалоговые окна, multi-slot save UI, SettingsScene и procedural ordinary regions ещё не реализованы.
