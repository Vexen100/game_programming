...
- обычная `RegionScene` проверяет, что player spawn, стартовые enemies, outposts, supply caches, NPCs и patrol route tiles достижимы;
...
Для idle/walk и visual-only attack анимации `ResourceManager` умеет загружать frame PNG по пути `entities/{animation_key}/{state}_{direction}_{frame_index}.png`.

Если изображения нет, используется placeholder surface с fallback-цветом из `Renderable` или tile color.

Если animation frame отсутствует, `ResourceManager` возвращает `None`, чтобы `RenderSystem` откатился к static `Sprite`.

Это не `AnimationManager`, не sound loading и не asset manifest.

Сложность: простая.

---

## AnimationSystem idle/walk/attack visual

`AnimationSystem` — runtime-система для минимальной idle/walk анимации и visual-only attack animation player/enemy.

Она обновляет `Animation.state`, `Animation.direction`, `Animation.frame_index` и `Animation.frame_timer` через deltaTime.

Сначала система consume-ит `AnimationRequest`, включает requested state, ставит visual lock timer и удаляет request.

Сейчас request используется для `attack`.

Пока `lock_timer > 0`, движение не перезаписывает attack animation.

После завершения lock система возвращает entity к обычному idle/walk выбору.

Если `Velocity` ненулевая, система включает `walk` и выбирает направление по скорости.

Если скорость нулевая, система включает `idle`, сбрасывает кадр и сохраняет направление через предыдущее значение или `FacingDirection`.

Система не рисует кадры и не знает про `ResourceManager`.

Player attack animation запрашивается при принятой melee-атаке.

Enemy attack animation запрашивается при старте enemy windup, но если enemy attack frames отсутствуют, работает static sprite fallback.

Visual-only attack animation не управляет damage timing, hitbox timing, cooldowns или AI.

Animation events, frame events, combo system, hitbox timing sync и `AnimationManager` не реализованы.

Combat rules, damage, HP, cooldowns, AI, influence, save schema и `GameState` не меняются.
...
Нарезанные walk frames используются минимальной idle/walk runtime-анимацией player/enemy.

Нарезанные player attack frames используются visual-only runtime-анимацией.

Enemy attack frames могут отсутствовать, тогда работает static enemy sprite fallback.

Это не `AnimationManager`, не animation events, не hitbox timing sync, не asset manifest, не sound pipeline и не редактор assets.
...
- уничтожение supply cache растёт через `SupplyCache.destroy_progress += dt`, пока игрок рядом, рядом нет живых врагов-охранников и удерживается `E`;
...
- после достижения duration выставляется финальный state-флаг `Outpost.cleared`, `SupplyCache.destroyed` или `NPC.quest_completed`.
...
- уничтожение supply cache даёт маленький сдвиг `+4/-4`;
...
Supply cache alone тоже не открывает штурм.

Один outpost + один NPC quest без боевого вклада тоже не открывают штурм.

Два outposts + supply cache + два NPC quests без боевого вклада тоже не открывают штурм.

Ожидаемый минимальный путь к unlock сейчас: два outposts, supply cache, два NPC quests и combat contribution.
...
- `SupplyCacheSystem` для nearby guard check;
- `CaptureSystem` для nearby enemy check.

`OutpostSystem`, `SupplyCacheSystem` и `CaptureSystem` не используют spatial index как финальный источник истины. После broadphase они проверяют живого врага и точную дистанцию внутри system.
