...
```text
Player = entity_id + Position + Velocity + Collider + Renderable + Sprite + Animation + Health + PlayerControlled + AttackIntent + FacingDirection + AttackHitbox + MeleeAttack
```
...
```text
Enemy = entity_id + Position + Velocity + Collider + Renderable + Sprite + Animation + Health + Enemy + ChaseBehavior + MeleeAttack + AttackHitbox + EnemyAttackState (+ PatrolRoute)
```
...
Supply cache — отдельная активность обычной `RegionScene`, создаваемая через `EntityFactory.create_supply_cache()`.

Текущий набор компонентов:

```text
SupplyCache = entity_id + Position + Collider + Renderable + Sprite + SupplyCache
```

На текущем этапе склад снабжения:

- создаётся в `RegionScene` из `RegionLayout`;
- имеет стабильный строковый `key`;
- не имеет здоровья;
- не является врагом, аванпостом, NPC или точкой захвата;
- не даёт loot, inventory items, currency или resources;
- уничтожается через короткое удержание `E`, если игрок рядом и рядом нет живых врагов-охранников;
- сбрасывает `destroy_progress`, если игрок ушёл, отпустил `E` или рядом есть живой враг;
- публикует `SupplyCacheDestroyedEvent` при первом уничтожении;
- меняет влияние региона через существующую цепочку `EventBus -> InfluenceSystem`.

`SupplyCache.destroyed` остаётся финальным runtime state-флагом.

В save snapshot обычного региона сохраняются только `destroyed_supply_cache_keys`, а не ECS-компонент и не PyGame objects.

Supply cache не является `QuestSystem`, `Outpost`, `CapturePoint`, inventory, loot или economy mechanic.
...
`Sprite` сам не управляет кадрами animation. Idle/walk кадры игрока и врагов выбираются через отдельный runtime-компонент `Animation`.

---

## Animation

`Animation` — runtime-only ECS-компонент минимальной idle/walk и visual-only attack анимации.

Текущий формат:

```text
Animation = animation_key + state + direction + frame_index + frame_timer + frame_duration + lock_timer + lock_duration
```

`animation_key` выбирает группу кадров, например `player` или `enemy`.

`state` сейчас поддерживает `idle`, `walk` и visual-only `attack`.

`direction` поддерживает `down`, `up`, `left` и `right`.

`AnimationSystem` обновляет `state`, `direction`, `frame_index` и `frame_timer` через deltaTime.

Если entity движется, используется `walk` и направление берётся из `Velocity`.

Если entity стоит, используется `idle`, кадр сбрасывается на `0`, а направление берётся из предыдущего направления или `FacingDirection`.

`AnimationRequest` запрашивает краткую locked-анимацию, сейчас только `attack`.

`AnimationSystem` consume-ит `AnimationRequest`, включает attack state, ставит `lock_timer`, сбрасывает кадр на `0` и удаляет request.

Пока `lock_timer > 0`, движение не перезаписывает attack animation. После завершения lock entity возвращается к обычному idle/walk состоянию.

`RenderSystem` пробует взять кадр из `ResourceManager` по пути вида `assets/images/entities/{animation_key}/{state}_{direction}_{frame_index}.png`.
...
Animation PNG также проходят asset cleanup: chroma-green remnants удаляются, transparent pixels получают RGB `(0, 0, 0)`, а low-alpha colored noise не должен попадать в final frames.
...
Visual-only attack animation не заменяет slash effect, hit flash, damage popup или enemy telegraph.

`AnimationManager`, animation events, combo system и hitbox timing sync пока не реализованы.
...
Save snapshot обычного региона хранит defeated enemy indexes, cleared outpost keys, destroyed supply cache keys, completed NPC keys, player position и player health.

Save snapshot не сериализует ECS напрямую и не хранит PyGame objects.

`InfluenceSystem` меняет `player_influence` и `enemy_influence` при `EnemyKilledEvent`, `OutpostClearedEvent`, `SupplyCacheDestroyedEvent` и `QuestCompletedEvent`. Если влияние врага падает достаточно низко, выставляется `assault_unlocked`.
